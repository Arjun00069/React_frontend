...
const Donor = () => {
  const [donor1,setDonor] =useState([])
...
fetchDonor()
        ...
        let cnt=10;
        for(let i=0 ;i<donor1.length;i++ ){
             if(donor1[i].donor){
             rows.push({
           id:cnt,
           uid:donor1[i].donor._id,
           string:donor1[i].donor.name,
           Age:donor1[i].donor.age,
           "Contact Number":donor1[i].donor.phone,
           bloodgroup:donor1[i].donor.bloodgrp,
           Quantity:donor1[i].quantity,
           'Donation Year':new Date().getFullYear(),
              'Status':donor1[i].status,
             })
            cnt++;
            }}
