...
export const register = (userData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`${server}/v1/register`, userData, {
        withCredentials:true,
...
export const getAllHospitals = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${server}/admin/allhospital`);
    console.log(data);
...
export const deleteHospital = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(`${server}/admin/deletehospital/${id}`);
    // const { data1 } = await axios.get("http://localhost:4000/blood/v1/admin/allhospital");
    // dispatch({ type: ALL_HOSPITALS_SUCCESS, payload: data1.hospitals });
...
export const getHospitalDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${server}/admin/gethospital/${id}`);
